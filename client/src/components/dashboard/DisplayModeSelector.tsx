...
interface DisplayModeSelectorProps {
  widgetType: 'kpi' | 'chart' | 'gauge' | 'network' | 'table' | 'process-flow'
  value: 'single' | 'dual' | 'multiple'
  onChange: (mode: 'single' | 'dual' | 'multiple') => void
}
...
const DISPLAY_MODES = {
  kpi: [
    ...
    },
    {
      id: 'dual' as const,
      label: 'Dual Values',
      description: 'Display two metric values',
  ...
  'process-flow': [
    {
      ...
      description: 'One process flow',
      icon: <Target size={16} />
    },
    {
      id: 'multiple' as const,
      label: 'Multiple Processes',
      description: 'Multiple process flows',
      icon: <Grid3x3 size={16} />
    }
  ]
}
...
export function DisplayModeSelector({ widgetType, value, onChange }: DisplayModeSelectorProps) {
  const availableModes = DISPLAY_MODES[widgetType] || DISPLAY_MODES.kpi
    ...
    <div>
      ...
      <div className="mt-3 p-3 bg-slate-800/30 rounded-lg border border-slate-700/50">
        <div className="text-xs text-slate-400">
          ...
          {value === 'dual' && 'Display two related values side by side'}
