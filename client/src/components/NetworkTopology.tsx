...
export function NetworkTopology({ facilities, onFacilityClick }: NetworkTopologyProps) {
  ...
  const getFacilityPosition = (facility: Facility) => {
    const positions: Record<string, { x: number; y: number }> = {
      ...
      "Jeddah": { x: 25, y: 60 }, // West coast 
      "Mecca": { x: 22, y: 70 }, // Southwest of Jeddah
      "Medina": { x: 30, y: 35 }, // Northwest of Riyadh
      "Dammam": { x: 75, y: 52 }, // East coast
      "Al-Khobar": { x: 78, y: 62 }, // Southeast coast
      "Jubail": { x: 72, y: 38 }, // Northeast coast
      "Tabuk": { x: 25, y: 25 }, // Northwest
      "Ha'il": { x: 40, y: 30 }, // North-central
      "Abha": { x: 40, y: 75 }, // South
    };
